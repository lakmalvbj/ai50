...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        check_for_mines_or_safe = True

        while check_for_mines_or_safe:
            check_for_mines_or_safe = False
            new_safes = set()
            new_mines = set()
            for sentence in self.knowledge:
            ...
            self.knowledge[:] = [x for x in self.knowledge if x.count != 0 and len(x.cells) != 0]
    ...
    def make_random_move(self):
        ...
        """
        available_random_moves = self.all_cells.difference(self.mines).difference(self.moves_made)
        if len(available_random_moves):
            return random.choice(list(available_random_moves))
        return None